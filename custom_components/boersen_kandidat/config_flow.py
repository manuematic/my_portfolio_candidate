"""Config flow + Options-Flow für Börsenkandidaten."""
...
class BoersenKandidatConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                vol.Required(CONF_LISTE_NAME, default="Meine Kaufkandidaten"): selector.selector(
                    {"text": {"type": "text"}}
                ),
    ...
    @staticmethod
    @callback
    def async_get_options_flow(config_entry: config_entries.ConfigEntry) -> "BoersenKandidatOptionsFlow":
        return BoersenKandidatOptionsFlow()


# ── Options Flow (Kandidaten-Verwaltung) ──────────────────────────────────────

class BoersenKandidatOptionsFlow(config_entries.OptionsFlow):
