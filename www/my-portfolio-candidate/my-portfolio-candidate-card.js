...
class MyPortfolioCandidateCard extends HTMLElement {
  ...
  _render() {
    ...
    const title = this._config.title || "My Portfolio Candidate";
    ...
    const emptyHint = displayed.length === 0
      ? `<tr><td colspan="5" class="empty">
          ${toggle ? "Kein Kandidat hat aktuell den Zielkurs unterschritten." : "Noch keine Purchase Candidates konfiguriert."}
         </td></tr>`
      : "";
...
window.customCards.push({
  ...
  description: "Zeigt Purchase Candidates mit Zielkurs-Überwachung und Alarm-Markierung.",
  preview: false,
});
